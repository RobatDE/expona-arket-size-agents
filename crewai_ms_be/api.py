...
from crew import TAMResearchCrew
...
def kickoff_crew(job_id, companies: list[str], regions: list[str], products: list[str]):
    logger.info(f"Crew for job {job_id} is starting")

    results = None
    try:
        tam_research_crew = TAMResearchCrew(job_id)
        tam_research_crew.setup_crew(companies, regions, products)
        results = tam_research_crew.kickoff()
...
def run_crew():
    ...
    # Validation
    data = request.json
    if not data or 'companies' not in data or 'regions' not in data or 'products' not in data:
    ...
    thread = Thread(target=kickoff_crew, args=(job_id, companies, regions, products))
    thread.start()
